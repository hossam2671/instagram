...
function UserDetails({ user }) {
    ...
    useEffect(()=>{
      ...
      .then((res) => {
        res.data.follwing.includes(user._id)?setFollowed(true):setFollowed(false)
      });
    },[])
  ...
  return (
    <div className={style["userDetails"]}>
      ...
      <div className={style["userPosts"]}>
        {
           user.posts.length != 0 ? user.posts.slice(0,3).map((post)=>(
                <img key={post._id} src={`http://localhost:5000/${post.img}`}/>
            )):<div className={style["noPosts"]}>
                <i class="fa-solid fa-camera"></i>
                <h3>No posts yet</h3>
                <p>When {user.userName} shares photos and reels, you'll see them here.</p>
            </div>
        }
      </div>
      {
        followed ? <h5 onClick={unfollow} style={{backgroundColor:"#9a9a9a" , color:"black"}}>Following</h5> :
